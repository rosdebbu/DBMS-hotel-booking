...
export default function HotelDetail() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64 text-gray-400 animate-pulse text-lg">Loading hotel...</div>;
  }

  const hotel = rooms[0];
  if (!hotel) {
    return <div className="flex items-center justify-center h-64 text-gray-400">Hotel not found</div>;
  }
    ...
    <div>
      ...
      <div className="bg-[#2a2a2a] border-b border-gray-700">
        <div className="max-w-[1400px] mx-auto px-6 py-3 flex items-center gap-3">
          ...
          <button onClick={() => navigate('/')} className="bg-amber-600 hover:bg-amber-700 text-white font-bold text-sm px-6 py-2.5 rounded transition-colors">
            Modify Search
          </button>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-3">
        <p className="text-blue-400 text-xs">
          <span className="cursor-pointer hover:underline" onClick={() => navigate(-1)}>‹ Back</span>
          <span className="text-gray-500"> › {hotel.location}, {checkin || '29 Apr 2026'}, {checkout || '30 Apr 2026'}, 1 Room, 1 Guest</span>
        </p>
      </div>
          ...
          <div className="p-6 flex items-start justify-between border-b border-gray-700">
            <div>
              ...
              <p className="text-gray-400 text-sm flex items-center gap-1">
                ...
                <span className="text-blue-400 text-xs ml-2 cursor-pointer hover:underline">View on map</span>
              </p>
            </div>
            <div className="text-right">
              ...
              <div className="mt-1 bg-blue-600 text-white text-sm font-bold w-10 h-10 rounded-full flex items-center justify-center ml-auto">
                {ratingNum.toFixed(1)}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-[55%] p-4">
              <div className="relative rounded-lg overflow-hidden h-[350px]">
                ...
                <button onClick={prevImg} className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white w-10 h-10 rounded-full flex items-center justify-center transition-colors">
                  <ChevronLeft size={20} />
                </button>
                <button onClick={nextImg} className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white w-10 h-10 rounded-full flex items-center justify-center transition-colors">
              ...
              <div className="flex gap-2 mt-2">
                {GALLERY_IMAGES.slice(0, 4).map((img, i) => (
                  <div key={i} onClick={() => setGalleryIdx(i)}
                    className={`w-1/4 h-20 rounded-md overflow-hidden cursor-pointer border-2 transition-all ${galleryIdx === i ? 'border-blue-500' : 'border-transparent opacity-70 hover:opacity-100'}`}>
            ...
            <div className="lg:w-[45%] p-6 border-l border-gray-700">
              <div className="mb-4">
                <p className="text-amber-400 text-lg font-semibold">{rooms[0]?.room_type} Room</p>
                <p className="text-gray-400 text-sm">1 x Guest | 1 x Room</p>
              </div>
              ...
              <div className="space-y-2 mb-6">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2 text-gray-300 text-sm">
                    <span className="text-green-400">{amenity.icon}</span>
                    {amenity.text}
                  </div>
                ))}
                <p className="text-blue-400 text-xs cursor-pointer hover:underline ml-5">+ More Amenities</p>
              </div>
              ...
              <div className="flex gap-3">
                <button className="flex-1 border border-gray-500 text-white font-semibold py-3 rounded-md hover:bg-white/5 transition-colors">
                  Select Rooms
                </button>
                <button
                  onClick={() => navigate(`/book?hotel=${encodeURIComponent(hotel.hotel_name)}&room=${rooms[0]?.room_id}&checkin=${checkin}&checkout=${checkout}`)}
                  className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 rounded-md transition-colors"
                >
                  Book Now
                </button>
              </div>
              ...
              {rooms.map((room, idx) => (
                <div key={room.room_id} className="grid grid-cols-4 items-center px-6 py-4 border-b border-gray-800 hover:bg-white/5 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white text-sm font-medium">{room.room_type}</p>
                      <p className="text-gray-500 text-xs">Guest room</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-gray-300 text-sm">| Room Only</p>
                    <p className="text-green-400 text-xs">✓ Free Cancellation</p>
                  </div>
                  <div className="text-center">
                    <p className="text-white text-xl font-bold">₹{parseFloat(room.price_per_night).toLocaleString()}</p>
                    <p className="text-gray-500 text-xs">per night</p>
                  </div>
                  <div className="text-right">
                    <button
                      onClick={() => navigate(`/book?hotel=${encodeURIComponent(room.hotel_name)}&room=${room.room_id}&type=${encodeURIComponent(room.room_type)}&price=${room.price_per_night}&location=${encodeURIComponent(room.location)}&rating=${room.rating}&checkin=${checkin}&checkout=${checkout}`)}
