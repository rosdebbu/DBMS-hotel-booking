    ...
    <div>
      ...
      <div className="bg-[#0d1117]/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-[1400px] mx-auto px-6 py-3 flex flex-col md:flex-row items-center gap-3">
          <div className="flex-[2] glass-card rounded-lg px-4 py-2">
          ...
          <button onClick={() => navigate('/')}
            className="bg-gradient-to-r from-amber-500 to-orange-600 text-white font-bold text-sm px-6 py-2.5 rounded-lg transition-all hover:shadow-lg hover:shadow-amber-500/20">
            Modify Search
          </button>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white text-lg font-bold">
            <span className="text-amber-400">{sortedResults.length}</span> Properties in{' '}
            <span className="bg-gradient-to-r from-blue-400 to-violet-400 bg-clip-text text-transparent">{locationQuery || 'All Locations'}</span>
          </h2>
          <div className="relative">
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
              className="appearance-none glass-card text-gray-300 rounded-lg px-4 py-2 pr-8 text-sm cursor-pointer outline-none">
          ...
          <aside className="hidden lg:block w-[260px] shrink-0">
            <div className="glass-card rounded-xl p-5 sticky top-20">
              <input type="text" placeholder="Search hotel name..."
                className="w-full bg-white/5 border border-white/10 rounded-lg text-sm text-white placeholder-gray-600 px-3 py-2 mb-5 outline-none focus:border-blue-500/50" />
              ...
              <div className="bg-gradient-to-r from-emerald-500/10 to-green-500/10 border border-emerald-500/20 rounded-xl p-3 mb-5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-emerald-400 font-bold text-sm">Price Guarantee</p>
                </div>
              ...
              <div className="border-b border-white/5 pb-4 mb-4">
                ...
                {[{ label: '5 Star', count: 470 }, { label: '4 Star', count: 2742 }, { label: '3 Star', count: 3546 }].map((s) => (
                  <label key={s.label} className="flex items-center gap-2 text-gray-400 text-sm mb-2.5 cursor-pointer hover:text-white transition-colors">
                    <input type="checkbox" className="accent-blue-500 rounded" />
                    {s.label}
                    <span className="ml-auto text-gray-600 text-xs">{s.count}</span>
                  </label>
                ))}
              </div>
              ...
              <div className="border-b border-white/5 pb-4 mb-4">
                ...
                {['Excellent (4.2+)', 'Very Good (3.5+)', 'Good (3+)'].map((r) => (
                  <label key={r} className="flex items-center gap-2 text-gray-400 text-sm mb-2.5 cursor-pointer hover:text-white transition-colors">
                    <input type="checkbox" className="accent-blue-500 rounded" />
                    {r}
                  </label>
                ))}
              ...
              <div>
                ...
                {['Gateway of India', 'Marine Drive', 'Juhu Beach'].map((a) => (
                  <label key={a} className="flex items-center gap-2 text-gray-400 text-sm mb-2.5 cursor-pointer hover:text-white transition-colors">
                    <input type="checkbox" className="accent-blue-500 rounded" />
                    {a}
                  </label>
                ))}
          ...
          <div className="flex-1 space-y-4">
            {loading ? (
              ...
              <div className="glass-card rounded-xl p-16 text-center">
                ...
                <button onClick={() => navigate('/')} className="text-blue-400 hover:underline text-sm">← Back to search</button>
              </div>
            ) : (
              sortedResults.map((room, idx) => (
                <div key={room.room_id}
                  className="glass-card rounded-xl overflow-hidden flex flex-col md:flex-row hover:bg-white/[0.03] transition-all group">
                  {/* Image */}
                  <div className="relative w-full md:w-[280px] h-[200px] md:h-auto shrink-0 overflow-hidden">
                    <img src={HOTEL_IMAGES[idx % HOTEL_IMAGES.length]} alt={room.hotel_name}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    <div className="absolute top-3 left-3 bg-black/50 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1 rounded-full border border-white/10 flex items-center gap-1">
                      <Zap size={10} className="text-amber-400" /> GoAnywhere Choice
                    </div>
                  </div>
                  ...
                  <div className="flex-1 p-5 flex flex-col">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="w-1 h-5 bg-gradient-to-b from-blue-400 to-violet-400 rounded-full shrink-0"></span>
                    ...
                    <p className="text-gray-300 text-sm ml-3 mb-1">
                      <span className="text-gray-500">Room: </span><span className="font-medium text-white">{room.room_type}</span>
                    </p>
                    <p className="text-gray-600 text-sm ml-3 mb-3">Complimentary Wi-Fi</p>
                    <div className="flex flex-wrap gap-3 ml-3 mb-3">
                      {[{ icon: <Coffee size={11} />, label: 'Breakfast' }, { icon: <Wifi size={11} />, label: 'Wi-Fi' },
                        { icon: <Car size={11} />, label: 'Parking' }, { icon: <UtensilsCrossed size={11} />, label: 'Dining' }].map((a) => (
                        <span key={a.label} className="text-gray-500 text-[11px] flex items-center gap-1">{a.icon} {a.label}</span>
                      ))}
                      <span className="text-blue-400 text-[11px] cursor-pointer">+4 More</span>
                    </div>
                  ...
                  <div className="border-t md:border-t-0 md:border-l border-white/5 p-5 flex flex-col items-end justify-end min-w-[180px]">
                    ...
                    <button className="bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 text-xs font-semibold px-4 py-2 rounded-lg mb-2 transition-colors flex items-center gap-1.5 w-full justify-center">
                    ...
                    <button
                      ...
                      className="bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white text-sm font-bold px-6 py-2.5 rounded-lg transition-all w-full shadow-lg shadow-amber-500/10">
                      View Rooms
                    </button>
                  </div>
                </div>
