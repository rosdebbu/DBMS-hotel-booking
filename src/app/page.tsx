...
export default function Home() {
  ...
  const [guests] = useState('1 Room, 1 Guest');
  const router = useRouter();

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (location.trim()) {
      router.push(`/search?location=${encodeURIComponent(location)}&checkin=${checkIn}&checkout=${checkOut}`);
    }
  };
      ...
      <section className="hero-gradient px-6 pt-16 pb-24 relative">
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="max-w-2xl">
            ...
            <p className="text-gray-500 text-sm">Powered by real-time database • MySQL + Next.js</p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 mt-14 pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {DESTINATIONS.map((dest) => (
            <div
              key={dest.name}
              onClick={() => router.push(`/search?location=${dest.name}`)}
